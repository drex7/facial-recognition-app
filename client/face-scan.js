...
async function captureAndEncodeFace() {
  const videoElement = document.getElementById("video-element");
  console.log(`videoElement->> 1.`, videoElement);
  ...
  console.log(`videoElement->> 2.`, videoElement);
  ...
  .then((capturedFaceData) => {
      ...
      if (capturedFaceData) {
        ...
        return fetch("http://localhost:5000/user/store_biometric_data", {
          ...
          body: JSON.stringify({ faceData: btoa(String.fromCharCode.apply(null, capturedFaceData)) }),
        });
      } else {
